...
class BuildDescriptionReviewState extends State<BuildDescriptionReview> {
    ...
    child = [
      ...
      Query(
        ...
        builder: (result, {fetchMore, refetch}) {
          ...
          print(
              'rgetr ${result.data!['products']['items'][0]['reviews']['items'][0]['nickname']}');
                  ...
                  ? SizedBox(
                      ...
                      child: Center(
                          child: Text('No reviews yet.',
                              style:
                                  AppStyles.getRegularTextStyle(fontSize: 14))))
                  : ListView.separated(
                      shrinkWrap: true,
                      physics: const NeverScrollableScrollPhysics(),
                      padding: const EdgeInsets.symmetric(vertical: 20),
                      itemCount: result
                          .data!['products']['items'][0]['reviews']['items']
                          .length,
                      // itemCount: 2,
                      ...
                      itemBuilder: (context, index) => Column(
                        ...
                        children: [
                            ...
                            children: [
                              Text(
                                  result.data!['products']['items'][0]
                                      ['reviews']['items'][index]['nickname'],
                                  style: AppStyles.getMediumTextStyle(
                                      fontSize: 16)),
                              RatingBar.builder(
                                initialRating: result.data!['products']['items']
                                            [0]['reviews']['items'][index]
                                        ['average_rating'] /
                                    20,
                                minRating: 1,
                          ...
                          Text(
                              result.data!['products']['items'][0]['reviews']
                                  ['items'][index]['summary'],
                              style:
                                  AppStyles.getMediumTextStyle(fontSize: 14)),
                          const SizedBox(height: 5),
                          Text(
                              result.data!['products']['items'][0]['reviews']
                                  ['items'][index]['text'],
                              style:
                                  AppStyles.getRegularTextStyle(fontSize: 13)),
                        ],
                      ),
                    ),
              Center(
              ...
              child: Material(
                child: Container(
                  ...
                  padding: EdgeInsets.symmetric(
                      vertical: AppResponsive.isDesktop(context) ? 20 : 10),
                  decoration: BoxDecoration(
                      color: Colors.white,
                    ...
                    builder: (result, {fetchMore, refetch}) {
                      ...
                      return Form(
                        ...
                        child: ListView(
                          ...
                          padding: EdgeInsets.symmetric(
                              horizontal:
                                  AppResponsive.isDesktop(context) ? 40 : 20,
                              vertical:
                                  AppResponsive.isDesktop(context) ? 20 : 10),
                          children: [
                            Text("Rate this product",
                                style:
                                    AppStyles.getMediumTextStyle(fontSize: 20)),
                            const SizedBox(height: 20),
                            ListView.separated(
                              shrinkWrap: true,
                              itemCount: result
                                    ...
                                    onPressed: () {
                                      if (formKey.currentState!.validate()) {
                                        ...
                                        for (var i in ratings) {
                                          if (i == 0) {
                                            errorData =
                                                'Please fill all the rating fields';
                                            setState(() {});
                                            return;
                                          }
                                        }
                                        List<Map<String, String>> ratingsData =
                                            getRatingsData(
                                                ratings, result.data!);
                                        //print(ratingsData);
                                        //print(ratings);
  ...
  List<Map<String, String>> getRatingsData(List<double> ratings, Map data) {
    ...
    for (int i = 0; i < ratings.length; i++) {
      int ratingIndex = data['productReviewRatingsMetadata']['items'][i]
              ['values']
          .indexWhere((value) =>
              value['value'].toString() == ratings[i].round().toString());
      ratingsData[i] = {
        'id': data['productReviewRatingsMetadata']['items'][i]['id'],
        'value_id': data['productReviewRatingsMetadata']['items'][i]['values']
            [ratingIndex]['value_id']
      };
    }
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 10),
        HtmlWidget(detail,
            textStyle: AppStyles.getRegularTextStyle(
